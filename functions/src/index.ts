...
const callableOptions = { region, timeoutSeconds: 60, memory: '256MiB' as const }
...
function assertAdmin(request: CallableRequest<AdminRequest>) {
...
function requiredText(value: unknown, label: string) {
  ...
  if (!result) throw new HttpsError('invalid-argument', `ไม่พบ${label}`)
  return result
}

function requestKey(uid: string, requestId: unknown) {
  const id = requiredText(requestId, ' requestId')
  if (!/^[A-Za-z0-9_-]{8,100}$/.test(id)) throw new HttpsError('invalid-argument', 'requestId ไม่ถูกต้อง')
  return `${uid}_${id}`
}

function uniqueKeyFor(book: Pick<BookInput, 'title' | 'author'>) {
...
async function mutateBook(
  request: CallableRequest<AdminRequest>,
  action: 'create' | 'update' | 'archive' | 'restore',
) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (previousRequest.exists) {
      ...
      if (previousData?.action !== action) throw new HttpsError('already-exists', 'requestId นี้ถูกใช้กับรายการอื่นแล้ว')
    ...
    if (action !== 'create' && !previousSnapshot?.exists) throw new HttpsError('not-found', 'ไม่พบหนังสือที่ต้องการ')
    const previous = previousSnapshot?.data() ?? null

    let next: FirebaseFirestore.DocumentData
    ...
    const nextUniqueKey = uniqueKeyFor(next as BookInput)
    const nextUniqueRef = db.collection('bookUniqueKeys').doc(nextUniqueKey)
    const previousUniqueKey = previous ? uniqueKeyFor(previous as BookInput) : null
...
export const healthcheck = onCall(callableOptions, (request) => {
  const admin = assertAdmin(request as CallableRequest<AdminRequest>)
